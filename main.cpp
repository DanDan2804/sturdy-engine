...
bool CheckCredentials(const std::wstring& username, const std::wstring& password) {
	std::unordered_map<std::wstring, std::wstring> credentials = {
    	{ L"Daniel", L"123456" },
    	{ L"Fulano", L"101112" },
    	{ L"Ciclano", L"000666" }
    };
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_CREATE: {
            ...
            CreateWindowW(L"Static", L"UsuÃ¡rio:", WS_VISIBLE | WS_CHILD, 50, 50, 100, 25, hwnd, NULL, NULL, NULL);
        ...
        case WM_COMMAND: {
            if (HIWORD(wParam) == BN_CLICKED) {
                ...
                // Verificar se o botÃ£o de login foi clicado
                if (controlId == 1) {
                    // Obter os valores do usuÃ¡rio e senha
